...
<template>
    <div>
        ...
        >
            <div v-for="(user, index) in subscribers" :key="index">
                <div
                    v-if="user.subscriber_detail"
                    class=" flex flex-col items-center space-y-5 border py-2 text-center"
                >
                   <router-link   :to="{name:'compte',  params: {id : user.subscriber_detail.id,slug:'' }}" class="flex justify-center flex-col items-center">
                    <img
                        class="object-cover lg:h-44 lg:w-44 rounded-full h-36 w-36"
                        :src="user.subscriber_detail.avatar"
                        v-if="user.subscriber_detail.avatar"
                    />
                    ...
                    >
                        {{ user.subscriber_detail.firstname }} {{ user.subscriber_detail.lastname }}
                    </h4>
                    <p class="leading-3">
                        <h2 class="font-light text-sm text-primary-blue">
                            {{ user.subscriber_detail.email }}
                        </h2>
                    </p>
                    </div>
                    </router-link>
